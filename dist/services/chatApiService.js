...
export class ChatApiService {
    static API_BASE_URL = import.meta.env.VITE_SUPABASE_URL;
    static OPENAI_API_KEY = import.meta.env.VITE_OPENAI_API_KEY;
    /**
     ...
     */
    static async sendToGroq(message, context) {
        const response = await fetch('https://api.groq.com/openai/v1/chat/completions', {
            ...
            body: JSON.stringify({
                ...
                messages: [
                    {
                        role: 'system',
                        content: `You are TutorAI, a helpful educational assistant. ${context ? `Previous context: ${context.join(' ')}` : ''}`
                    },
                    {
                        role: 'user',
                        content: message
     ...
     * Send message to OpenAI API
     */
    static async sendToOpenAI(message, context) {
     ...
     */
    static async sendToMockAPI(message) {
        // Simulate API delay
        ...
        const hasQuizKeyword = quizKeywords.some(keyword => message.toLowerCase().includes(keyword));
     ...
     */
    static async handleQuizCommand(command, sessionId) {
        try {
     ...
     */
    static async generateQuizFromChat(request) {
