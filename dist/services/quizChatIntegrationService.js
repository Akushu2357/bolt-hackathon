export class QuizChatIntegrationService {
    static STORAGE_KEY = 'tutorAI_quiz_chat_context';
    /**
     * Store quiz context for chat integration
     */
    static storeQuizContext(context) {
        try {
     ...
     */
    static generateSuggestedPrompts(context) {
        const prompts = [];
     ...
     */
    static createInitialChatMessage(context) {
     ...
     */
    static formatQuizResultsForChat(quiz, selectedAnswers, score, gradingResults) {
        const weakAreas = [];
        const strengths = [];
    ...
    static extractConcept(questionText) {
    ...
    static formatUserAnswer(userAnswer, question) {
    ...
    static formatCorrectAnswer(correctAnswer, question) {
        if (question.type === 'open_ended') {
