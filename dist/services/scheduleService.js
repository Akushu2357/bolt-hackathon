...
export class ScheduleService {
     ...
     */
    static async fetchScheduleItems(user) {
        try {
     ...
     */
    static async fetchScheduleItemsByDate(user, date) {
        try {
            const { data, error } = await supabase
                .from('schedule')
                .select('*')
     ...
     */
    static async fetchUpcomingScheduleItems(user, limit) {
        try {
     ...
     */
    static async createScheduleItem(user, item) {
        try {
            const { data, error } = await supabase
                .from('schedule')
                .insert({
     ...
     */
    static async updateScheduleItem(user, itemId, updates) {
        try {
            const { data, error } = await supabase
                .from('schedule')
                .update({
     ...
     */
    static async toggleScheduleItemCompletion(user, itemId) {
        try {
     ...
     */
    static async deleteScheduleItem(user, itemId) {
        try {
     ...
     */
    static async getScheduleStats(user) {
        try {
            const today = new Date().toISOString().split('T')[0];
            // Get all items
